...
#if(ZRBUG_LOG == 1)
# define ZRLog(fmt, ...) NSLog((@"[函数名:%s]" " [第 %d 行]\n" fmt),  __FUNCTION__, __LINE__, ##__VA_ARGS__);
#else
# define ZRLog(...);
#endif
...
typedef NS_ENUM(NSInteger,apiType) {
    ...
    ZRRequestTypeDefault,   //默认类型
    ZRRequestTypeRefresh,   //重新请求 （有缓存，不读取，重新请求）
    ZRRequestTypeLoadMore,  //加载更多
    ZRRequestTypeDetail,    //详情
    ZRRequestTypeOffline,   //离线    （有缓存，不读取，重新请求）
    ZRRequestTypeCustom     //自定义
...
typedef void (^requestConfig)(ZRURLRequest *request);
...
@interface ZRURLRequest : NSObject
